...
/**
 * A Java object representation of an Open Graph enabled webpage. A simplified
 * layer over a Hastable.
 *
 * @author Callum Jones
 */
public class OpenGraph {
	...
	private AtomicInteger redirectCounter;
	...
	static {
		...
		BASE_TYPES.put("person", new String[] { "actor", "athlete", "author", "director", "musician", "politician",
				"profile", "public_figure" });
		BASE_TYPES.put("place", new String[] { "city", "country", "landmark", "state_province" });
		BASE_TYPES.put("product",
				new String[] { "album", "book", "drink", "food", "game", "movie", "product", "song", "tv_show" });
	...
	public OpenGraph() {
		...
		hasChanged = false;
		isImported = false;
		redirectCounter = new AtomicInteger(3);
	}
	 ...
	 * @param ignoreSpecErrors Set this option to true if you don't wish to have an
	 *                         exception throw if the page does not conform to the
	 *                         basic 4 attributes
	 * @throws java.io.IOException If a network error occurs, the HTML parser will
	 *                             throw an IO Exception
	 * @throws java.lang.Exception A generic exception is throw if the specific page
	 *                             fails to conform to the basic Open Graph standard
	 *                             as define by the constant REQUIRED_META
	 */
	public OpenGraph(String url, boolean ignoreSpecErrors) throws java.io.IOException, Exception {
		this();
		this.load(url, ignoreSpecErrors);
	}
	
	private int load(String url, boolean ignoreSpecErrors) throws java.io.IOException, Exception {
		isImported = true;

		// download the (X)HTML content, but only up to the closing head tag. We do not
		// want to waste resources parsing irrelevant content
		...
		int statusCode = siteConnection.getResponseCode();

		if (statusCode == HttpURLConnection.HTTP_MOVED_PERM || statusCode == HttpURLConnection.HTTP_MOVED_TEMP) {
			if (redirectCounter.decrementAndGet() < 0) {
				throw new Exception("Maximum redirect count exceeded.");
			}
			String location = siteConnection.getHeaderField("Location");
			if (location == null) {
				throw new Exception("The Site reponsed 30X without location header.");
			}
			return load(location, ignoreSpecErrors);
		}
		...
		for (TagNode metaElement : metaData) {
			for (OpenGraphNamespace namespace : pageNamespaces) {
				...
				if (target != null && metaElement.getAttributeByName(target).startsWith(namespace.getPrefix() + ":")) {
					setProperty(namespace, metaElement.getAttributeByName(target),
	...
	/**
	 * Gets the charset for specified connection. Content Type header is parsed to
	 * get the charset name.
	 *
	 * @param connection the connection.
	 * @return the Charset object for response charset name; if it's not found then
	 *         the default charset.
	 */
	...
	/**
	 * Extract the charset name form the content type string. Content type string is
	 * received from Content-Type header.
	 *
	...
	public String[] toHTML() {
		...
		for (ArrayList<MetaElement> elements : metaAttributes.values()) {
			for (MetaElement element : elements)
				returnHTML.add("<meta property=\"" + element.getNamespace() + ":" + element.getProperty()
	...
	public String[] toXHTML() {
		...
		for (ArrayList<MetaElement> elements : metaAttributes.values()) {
			for (MetaElement element : elements)
				returnHTML.add("<meta name=\"" + element.getNamespace().getPrefix() + ":" + element.getProperty()
						+ "\" content=\"" + element.getContent() + "\" />");
		}
	...
	/**
	 * Test if the object has been modified by setters/deleters. This is only
	 * relevant if this object initially represented a web page
	 * 
